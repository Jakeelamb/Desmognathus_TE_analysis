...
is to determine whether the frozen TE outputs already present in this repo are
internally consistent, traceable, and suitable for manuscript use and for the
...
The current TE data are in good enough shape to use for the paper and for the
current path-analysis pipeline, with a few explicit caveats.
...
exactly from the frozen repo-local source tables without rerunning the original
HPC analyses.

The ectopic support cleanup performed during this audit did not change current
panel membership. The TE + genome and TE + genome + ectopic panel sizes remain
...
- `Shannon_Diversity` and `Pielou_Evenness` in
...
- The thresholded divergence table used by the path-analysis features is
  complete for the current scope: `34` species, `9` TE orders, `306` rows, no
...
- The legacy overlap datasets and the current medium-plus panel files still
  agree on species membership for the core TE families.

## Current Warnings

### 1. Simpson definition needs explicit documentation

`Simpson_Diversity` in `results/data/diversity_order_stats.csv` is not the raw
Gini-Simpson value computed directly from the order table. It is exactly the raw
value multiplied by `100/99` for every species. That looks like a
finite-sample-style correction applied to percentage-scaled totals.

Interpretation:

- this is a definitional issue, not a corruption issue
- `order_pielou` remains the cleaner evenness variable for the path models
- `order_simpson` should not be described casually in the paper without
  documenting this correction

### 2. One ectopic row has a missing ratio
...
### 3. One ectopic row has `Complete = unknown`
...
`ectopic_complete_fraction` is discussed in the manuscript.
...
For paper use and path-analysis use, treat these as the canonical TE layer:
...
- for overlapping provenance fields inside `path_input_master.csv`, prefer the
  ...
  provenance rather than the older overlap-builder provenance
...
- the derived path-analysis TE tables are reproducible from those frozen inputs
...
- Use `order_pielou` rather than `order_simpson` in narrative summaries unless
  the Simpson correction is explicitly documented.
- If `ectopic_n_elements` or `ectopic_complete_fraction` will be emphasized in
  the manuscript, report their current semantics explicitly:
...
- If you want to go one layer deeper, the next audit target is the upstream
  provenance chain from assemblies/HPC jobs into the frozen repo-local summary
  tables.
