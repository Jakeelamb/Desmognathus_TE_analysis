...
#
# The analysis ranks all ten testable Markov-equivalence classes on genome size,
# nucleus size, and cell size. The eleventh (saturated) class has no d-separation
...
resolve_review_input <- function(relative_path) {
  candidates <- c(
  ...
  if (!length(present)) return(candidates[[1]])
  present[[1]]
}

trait_path <- file.path(input_dir, "primary_traits_all_finalized.csv")
measurement_path <- file.path(input_dir, "measurement_bootstrap_traits_250.csv")
focal_tree_path <- resolve_review_input(
  "input_data/phylogeny/desmo900dated_test.tre"
)
published_main_path <- resolve_review_input(
  "results/data/corrected/phylogeny/stewart_wiens_2025_main_analysis18_v1.nwk"
)
published_bootstrap_path <- resolve_review_input(
...
alpha <- 0.05
competitive_delta <- 2
node_labels <- c(g = "genome_size", n = "nucleus_size", c = "cell_size")
model_order <- c(
  "independent",
  "genome_nucleus_only",
  "nucleus_cell_only",
...
validate_traits <- function(source, expected_species = NULL) {
  required <- c(
    "species", "tree_tip", "genome_size_pg", "nucleus_area_um2", "cell_area_um2"
  )
  ...
  numeric <- source[, c("genome_size_pg", "nucleus_area_um2", "cell_area_um2")]
...
prepare_model_data <- function(source, tree) {
  ...
  data <- data.frame(
    g = zscore(log10(source$genome_size_pg)),
...
exhaustive_model_set <- function() {
  define_model_set(
    independent = c(),
    genome_nucleus_only = c(n ~ g),
    nucleus_cell_only = c(c ~ n),
    genome_cell_only = c(c ~ g),
    nucleus_bridge = c(n ~ g, c ~ n),
    nucleus_collider = c(n ~ g + c),
    cell_bridge = c(c ~ g, n ~ c),
    cell_collider = c(c ~ g + n),
    genome_bridge = c(g ~ n, c ~ g),
    genome_collider = c(g ~ n + c)
  )
}

user_mechanism_model_set <- function() {
  define_model_set(
    genome_to_nucleus_to_cell = c(n ~ g, c ~ n),
    cell_to_nucleus_to_genome = c(n ~ c, g ~ n),
    nucleus_to_genome_and_cell = c(g ~ n, c ~ n)
  )
}
...
single_mechanism_model_set <- function(mechanism) {
  switch(
    mechanism,
    genome_to_nucleus_to_cell = define_model_set(
      genome_to_nucleus_to_cell = c(n ~ g, c ~ n)
    ),
    cell_to_nucleus_to_genome = define_model_set(
      cell_to_nucleus_to_genome = c(n ~ c, g ~ n)
    ),
    nucleus_to_genome_and_cell = define_model_set(
      nucleus_to_genome_and_cell = c(g ~ n, c ~ n)
    ),
...
fit_phylogenetic_regressions <- function(source, tree, evolutionary_model = "lambda") {
  prepared <- prepare_model_data(source, tree)
  specifications <- list(
    genome_to_nucleus = n ~ g,
    nucleus_to_genome = g ~ n,
    genome_to_cell = c ~ g,
    cell_to_genome = g ~ c,
    nucleus_to_cell = c ~ n,
    cell_to_nucleus = n ~ c,
    genome_and_nucleus_to_cell = c ~ g + n,
    genome_and_cell_to_nucleus = n ~ g + c,
    nucleus_and_cell_to_genome = g ~ n + c
  )
  rows <- list()
...
raw_correlations <- function(source) {
  specifications <- list(
    genome_nucleus = c("genome_size_pg", "nucleus_area_um2"),
    genome_cell = c("genome_size_pg", "cell_area_um2"),
...
simulate_traits <- function(tree, generating_class, residual_regime, coefficients) {
  ...
  if (generating_class == "independent") {
    ...
    g <- first
    n <- combine(g, second_error, coefficients[["genome_to_nucleus"]])
    c <- combine(n, third_error, coefficients[["nucleus_to_cell"]])
  } else if (generating_class == "cell_bridge") {
    g <- first
    c <- combine(g, second_error, coefficients[["genome_to_cell"]])
    n <- combine(c, third_error, coefficients[["cell_to_nucleus"]])
  } else if (generating_class == "genome_bridge") {
    n <- first
    g <- combine(n, second_error, coefficients[["nucleus_to_genome"]])
    c <- combine(g, third_error, coefficients[["genome_to_cell"]])
  } else {
...
for (mechanism in c(
  "genome_to_nucleus_to_cell",
  "cell_to_nucleus_to_genome",
  "nucleus_to_genome_and_cell"
)) {
  result <- fit_path(
    traits,
    focal_tree,
...
required_coefficients <- c(
  "genome_to_nucleus", "nucleus_to_genome", "genome_to_cell",
...
simulation_classes <- c("independent", "nucleus_bridge", "cell_bridge", "genome_bridge")
...
manifest <- list(
  ...
  genome_size_units = "pg; D. fuscus anchored at 16.36 pg",
  genome_calibration_note = paste(
    "A shared multiplicative genome calibration does not change standardized",
    "log-scale path results; image-derived IOD measurement uncertainty does."
  ),
  ...
  common_user_mechanism_basis_claim = "genome_size independent of cell_size conditional on nucleus_size"
)
write_json(
  manifest,
...
cat("Completed exhaustive cell–nucleus–genome path analysis.\n")
