#!/usr/bin/env Rscript
# Compatibility wrapper for the canonical PGLS workflow.
...
canonical_script <- file.path(script_dir, "pgls_analysis.R")
if (!file.exists(canonical_script)) {
  stop("Canonical PGLS script not found: ", canonical_script, call. = FALSE)
}

message("Delegating to canonical workflow: ", canonical_script)
status <- system2("Rscript", canonical_script)
if (!identical(status, 0L)) {
  stop("Canonical PGLS workflow failed with exit status ", status, call. = FALSE)
}
