...
def test_compact_inputs_reproduce_canonical_diversity() -> None:
    ...
    assert diversity.groupby(["te_level", "composition_mode"]).size().to_dict() == {
        ("order", "classified_conditional"): 34,
        ("order", "mass_aware_unresolved_bin"): 34,
        ("superfamily", "classified_conditional"): 34,
        ("superfamily", "mass_aware_unresolved_bin"): 34,
    }
...
def test_te_relative_iod_overlap_uses_exact_canonical_diversity_rows() -> None:
    overlap = pd.read_csv(
        ROOT / "analyses/03_phylogenetic_path/data/te_relative_iod_overlap.csv"
    )
    ...
    diversity = pd.read_csv(
        ROOT / "analyses/01_transposable_elements/data/te_diversity.csv"
    )
    ...
    te34_species = set(
        te34.loc[te34["te_resource_panel34_v1"].eq(True), "species"]
    )
    path24_species = set(
        path24.loc[path24["path_analysis_panel24_v1"].eq(True), "species"]
    )
    ...
    source_rows = diversity.loc[
        diversity["te_level"].eq("superfamily")
...
def test_mass_aware_mode_counts_unresolved_as_an_accounting_bin() -> None:
    composition = pd.DataFrame(
        {"feature_a": [0.75], "feature_b": [0.25]},
        index=pd.Index(["example"], name="species"),
    )
    mass = pd.DataFrame(
        {
            "species": ["example"],
            "order_retained_fraction": [0.8],
            "order_unresolved_fraction": [0.2],
        }
    )

    mass_aware = REPLAY.add_unresolved_accounting_bin(composition, mass, "order")
    classified_metrics = REPLAY.diversity_metrics(composition.loc["example"])
    mass_aware_metrics = REPLAY.diversity_metrics(mass_aware.loc["example"])

    assert mass_aware.columns.tolist() == [
        "feature_a",
        "feature_b",
        REPLAY.UNRESOLVED_ACCOUNTING_BIN,
    ]
    assert np.isclose(mass_aware.loc["example", REPLAY.UNRESOLVED_ACCOUNTING_BIN], 0.2)
    assert np.isclose(mass_aware.loc["example"].sum(), 1)
    assert mass_aware_metrics["observed_richness"] == (classified_metrics["observed_richness"] + 1)
